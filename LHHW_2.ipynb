# ================================================================
#   ALDOL CONDENSATION: LHHW SINGLE-SITE (All Species ads) MODEL FITTING
...
#
print("\n\n=====================================================================")
print("--- PART 1: Fitting LHHW (All Species ads) Parameters Per Temperature ---")
print("=====================================================================")

def fitting_rate_law_lhhw_ss_prod(t, XA, T, M, CA0, k_s, K_A, K_B, K_C, K_D):
    """
    Defines the ODE for the LHHW single-site model with reactant and product adsorption.
    Rate = (ks * KA * KB * CA * CB) / (1 + KA*CA + KB*CB + KC*CC + KD*CD)
...
def sum_of_squared_errors_lhhw_ss_prod(params, target_temp):
    """
...
print("\n--- Per-Temperature LHHW (All Species ads) Fit Results ---")
unique_temps = sorted(list(set(raw_data['Temperature (K)'])))
for temp in unique_temps:
    print(f"Fitting for {temp} K...")
    result = minimize(
        fun=lambda p: sum_of_squared_errors_lhhw_ss_prod(p, target_temp=temp),
        x0=initial_guess,
        method='L-BFGS-B',
        bounds=bounds
    )
...
print("\n--- Post-Fit Arrhenius Analysis ---")
temps_K = np.array(list(fitted_ks_map.keys())); fitted_ks_array = np.array(list(fitted_ks_map.values()))
...
fig1.suptitle('PART 1: LHHW (All Species ads) Model Fit (Per-Temp)', fontsize=18)
...
fig2.suptitle('PART 2: LHHW (All Species ads) Predictive Model Validation', fontsize=18)
