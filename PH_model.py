...
# NOTE: Ensure your CSV file has 'Ratio' column values like '1:3', '1:4', '1:5'
file_path = '/content/BTP data  - Sheet1.csv'
...
# --- MODIFICATION START: Updated for Aldol Condensation ---
# --- Calculated initial conditions from reactant weights for Aldol Condensation ---
# Molar Masses: Acetone = 58.08 g/mol, Benzaldehyde = 106.12 g/mol
# Assumed Total Volume = 1.0 L
# Limiting Reactant (A) is Benzaldehyde.
# M is the Molar Ratio of Acetone / Benzaldehyde.
# CA0 corresponds to Benzaldehyde concentration.
initial_conditions = {
    # Molar Ratio Key: [CA0 (mol/L), M (Acetone/Benzaldehyde)]
    3: [0.716, 2.98],  # For 1:3 ratio (76g Benz / 124g Acetone)
    4: [0.594, 4.00],  # For 1:4 ratio (63g Benz / 138g Acetone)
    5: [0.481, 5.22]   # For 1:5 ratio (51g Benz / 146g Acetone)
}

# --- Structure the data for analysis ---
experimental_runs = {}
for (ratio, temp), group in raw_data.groupby(['Ratio', 'Temperature (K)']):
    # Assumes ratio format is 'X:Y', takes the 'Y' part
...
print("\n\n=====================================================================")
print("--- PART 1: Fitting PH Model for k_433, k_443, and k_453 ---")
print("=====================================================================")

def fitting_rate_law_ph(t, XA, T, M, CA0, k_433, k_443, k_453):
    """
...
def sum_of_squared_errors_ph(params):
    ...
    for run_data in experimental_runs.values():
        ...
        sol = solve_ivp(
            fun=lambda t, y: fitting_rate_law_ph(t, y, run_data['T'], run_data['M'], run_data['CA0'], k_433, k_443, k_453),
...
k433_fit_ph, k443_fit_ph, k453_fit_ph = result_ph.x
fitted_ks_map_ph = {433: k433_fit_ph, 443: k443_fit_ph, 453: k453_fit_ph}
    ...
    for run_data in experimental_runs.values():
        ...
        sol = solve_ivp(
            fun=lambda t, y: fitting_rate_law_ph(
                ...
                k433_fit_ph, k443_fit_ph, k453_fit_ph
            ),
...
print(f"Calculated Ea: {Ea_fit_ph/1000:.2f} kJ/mol | Calculated A: {A_fit_ph:.2e} | R-squared: {r_value_ph**2:.4f}")

# Visualization of Part 1
fig1, axes1 = plt.subplots(1, 3, figsize=(20, 6), sharey=True)
fig1.suptitle('PART 1: PH Model Fit for Aldol Condensation', fontsize=18)
plots_data = {433: [], 443: [], 453: []}
for run_data in experimental_runs.values():
...
def predictive_rate_law_ph(t, XA, T, M, CA0, A, Ea):
    ...
    dxdt = k_predicted * CA0 * (1 - XA) * (M - XA)
    return dxdt if dxdt > 0 else 0
...
fig2.suptitle('PART 2: PH Predictive Model Validation for Aldol Condensation', fontsize=18)
...
print(f"\nOverall SSE (sum of squared error) of the Predictive PH Model (Parity): {ss_res_total:.5f}")
