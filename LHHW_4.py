# ================================================================
#   ALDOL CONDENSATION: LHHW DUAL-SITE (All Species ads) MODEL FITTING
...
file_path = 'BTP data  - Sheet1.csv'  # <-- Make sure this file path is correct
...
# --- Initial Conditions for Aldol Condensation ---
initial_conditions = {
    # Molar Ratio Key: [CA0 (mol/L), M (Acetone/Benzaldehyde)]
    3: [0.716, 2.98],  # For 1:3 ratio
    4: [0.594, 4.00],  # For 1:4 ratio
    5: [0.481, 5.22]   # For 1:5 ratio
}
...
print("--- Data has been structured for Aldol Condensation analysis. ---")


################################################################################
# --- PART 1: FITTING LHHW DUAL-SITE MODEL (WITH PRODUCT ADSORPTION) ---
################################################################################
#
#   *** MODIFICATION START ***
...
#
print("\n\n=====================================================================")
print("--- PART 1: Fitting LHHW Dual-Site (All Species ads) Parameters Per Temperature ---")
print("=====================================================================")

def fitting_rate_law_lhhw_ds_prod(t, XA, T, M, CA0, k_s, K_A, K_B, K_C, K_D):
    """
    Defines the ODE for the LHHW dual-site model with reactant and product adsorption.
    Rate = (ks * KA * KB * CA * CB) / (1 + KA*CA + KB*CB + KC*CC + KD*CD)^2
    Note: This function now takes *single* k_s and all K values.
    """
    C_A = CA0 * (1 - XA)
    C_B = CA0 * (M - XA)
    C_C = CA0 * XA
    C_D = CA0 * XA # Assuming 1:1 stoichiometry for product formation

    numerator = k_s * K_A * K_B * C_A * C_B
    denominator = (1 + K_A * C_A + K_B * C_B + K_C * C_C + K_D * C_D)**2
...
def sum_of_squared_errors_lhhw_ds_prod(params, target_temp):
    """
    Objective function to minimize FOR A SINGLE TEMPERATURE.
    """
    total_error = 0
    k_s, K_A, K_B, K_C, K_D = params # 5 parameters to fit
    ...
    for run_data in experimental_runs.values():
        ...
        sol = solve_ivp(
            fun=lambda t, y: fitting_rate_law_lhhw_ds_prod(
                t, y, run_data['T'], run_data['M'], run_data['CA0'],
                k_s, K_A, K_B, K_C, K_D # Pass the 5 params directly
            ),
...
# --- Optimization Loop ---
initial_guess = [0.1, 0.5, 0.5, 0.1, 0.1] # Initial guess for [ks, KA, KB, KC, KD]
bounds = [(1e-6, None)] * 5                # Bounds for [ks, KA, KB, KC, KD]

# Dictionaries to store the per-temperature results
fitted_params_map = {} # Will store {433: {'ks':..., 'KA':..., 'KB':..., 'KC':..., 'KD':...}, ...}
fitted_ks_map = {}     # Will store {433: ks_fit, 443: ks_fit, ...}

print("\n--- Per-Temperature LHHW Dual-Site (All Species ads) Fit Results ---")
unique_temps = sorted(list(set(raw_data['Temperature (K)'])))
for temp in unique_temps:
    print(f"Fitting for {temp} K...")
    result = minimize(
        fun=lambda p: sum_of_squared_errors_lhhw_ds_prod(p, target_temp=temp),
        x0=initial_guess,
        method='L-BFGS-B',
        bounds=bounds
    )
    ...
    if result.success:
        ks_fit, KA_fit, KB_fit, KC_fit, KD_fit = result.x
        fitted_params_map[temp] = {
            'ks': ks_fit, 'KA': KA_fit, 'KB': KB_fit, 'KC': KC_fit, 'KD': KD_fit
        }
        fitted_ks_map[temp] = ks_fit # For Arrhenius plot
        print(f"  k_s: {ks_fit:.5f} | K_A: {KA_fit:.4f} | K_B: {KB_fit:.4f} | K_C: {KC_fit:.4f} | K_D: {KD_fit:.4f}")
    else:
        print(f"  ERROR: Fit failed for {temp} K.")
        fitted_params_map[temp] = {
            'ks': np.nan, 'KA': np.nan, 'KB': np.nan, 'KC': np.nan, 'KD': np.nan
        }
...
fig1.suptitle('PART 1: LHHW Dual-Site (All Species ads) Model Fit (Per-Temp)', fontsize=18)
...
for i, temp in enumerate(plots_data.keys()):
    ...
    ks_plot = temp_params['ks']
    KA_plot, KB_plot = temp_params['KA'], temp_params['KB']
    KC_plot, KD_plot = temp_params['KC'], temp_params['KD']
    ...
    for run_data in plots_data[temp]:
        ...
        sol_plot = solve_ivp(
            fun=lambda t, y: fitting_rate_law_lhhw_ds_prod(
                ...
                ks_plot, KA_plot, KB_plot, KC_plot, KD_plot # Use the per-temp params
            ),
...
axes1[0].set_ylabel('Conversion of Benzaldehyde (fraction)', fontsize=12)
plt.ylim(0, 1); plt.tight_layout(rect=[0, 0, 1, 0.95]); plt.show()


################################################################################
# --- PART 2: VALIDATING THE LHHW DUAL-SITE MODEL (WITH PRODUCT ADSORPTION) ---
################################################################################
#
#   *** MODIFICATION START ***
#   The predictive model uses k_s from Arrhenius, but the
#   K_A, K_B, K_C, K_D fitted for that specific temperature.
#
print("\n\n============================================================")
print("--- PART 2: Validating Model with A_s and Ea ---")
print("============================================================")

def predictive_rate_law_lhhw_ds_prod(t, XA, T, M, CA0, A_s, Ea, K_A, K_B, K_C, K_D):
    """
    Predictive rate law using Arrhenius for k_s, and per-temp K values
    """
    k_s_predicted = A_s * np.exp(-Ea / (R * T)) # k_s from Arrhenius
    C_A = CA0 * (1 - XA); C_B = CA0 * (M - XA)
    C_C = CA0 * XA; C_D = CA0 * XA

    # K_A, K_B, K_C, K_D are the fitted values for this temperature
...
fig2.suptitle('PART 2: LHHW Dual-Site (All Species ads) Predictive Model Validation', fontsize=18)

# *** NOTE: We reset all_exp_points and all_model_points here for Part 3 ***
all_exp_points, all_model_points = [], []

for i, temp in enumerate(plots_data.keys()):
    ax = axes2[i]; ax.set_title(f'Temperature: {temp} K', fontsize=14)

    # Get the fitted K values for this temp
    temp_params = fitted_params_map[temp]
    KA_plot, KB_plot = temp_params['KA'], temp_params['KB']
    KC_plot, KD_plot = temp_params['KC'], temp_params['KD']
    ...
    for run_data in plots_data[temp]:
        ...
        t_plot = np.linspace(0, max(time_exp) if max(time_exp) > 0 else 1, 100)
        sol_plot = solve_ivp(
            fun=lambda t, y: predictive_rate_law_lhhw_ds_prod(
                t, y, temp, run_data['M'], run_data['CA0'],
                As_fit, Ea_fit, KA_plot, KB_plot, KC_plot, KD_plot # Use per-temp K's
            ),
            t_span=[0, max(time_exp) if max(time_exp) > 0 else 1],
            y0=[0],
            method='LSODA',
    ...
    ax.set_xlabel('Time (minutes)', fontsize=12); ax.grid(True); ax.legend()
axes2[0].set_ylabel('Conversion of Benzaldehyde (fraction)', fontsize=12)
plt.ylim(0, 1); plt.tight_layout(rect=[0, 0, 1, 0.95]); plt.show()
...
#
#   *** MODIFICATION START ***
#   The results table is updated to pull all four per-temperature
#   K values (KA, KB, KC, KD) from our `fitted_params_map`.
#
print("\n\n=======================================================")
print("--- PART 3: Final LHHW Dual-Site (All Species ads) Results Summary ---")
print("=======================================================")

# --- MODIFICATION: Add dictionaries to store per-temp points ---
per_temp_exp_points = {temp: [] for temp in fitted_params_map.keys()}
per_temp_model_points = {temp: [] for temp in fitted_params_map.keys()}
# --- END MODIFICATION ---
...
for name, run_data in experimental_runs.items():
    ...
    kc_fit_val = fitted_params_map[temp]['KC']
    kd_fit_val = fitted_params_map[temp]['KD']
    ...
    sol_exp_points = solve_ivp(
        fun=lambda t, y: predictive_rate_law_lhhw_ds_prod(
            ...
            As_fit, Ea_fit, ka_fit_val, kb_fit_val, kc_fit_val, kd_fit_val
        ),
    ...
    # Populate all points for Overall R-squared
    all_exp_points.extend(conv_exp_with_zero)
    all_model_points.extend(conv_model)

    # --- MODIFICATION: Populate per-temp dictionaries ---
    per_temp_exp_points[temp].extend(conv_exp_with_zero)
    per_temp_model_points[temp].extend(conv_model)
    # --- END MODIFICATION ---
...
# --- MODIFICATION: Calculate per-temp SSE/R^2 and modify print loop ---
print("\n--- Per-Temperature Fitted Adsorption Constants & Model Fit (±5%) ---")
for temp, params in fitted_params_map.items():
    # Calculate stats for this temperature using the PREDICTIVE model
    exp_pts = np.array(per_temp_exp_points[temp])
    mod_pts = np.array(per_temp_model_points[temp])

    temp_ss_res = np.sum((exp_pts - mod_pts)**2)
    temp_ss_tot = np.sum((exp_pts - np.mean(exp_pts))**2)
    temp_r_squared = 1 - (temp_ss_res / temp_ss_tot) if temp_ss_tot > 0 else 0
    ...
    # Print all info for this temp
    print(f"Temp: {temp} K")
    print(f"  K_A (Benzaldehyde): {ka_val:.4f} ± {ka_val*error_margin:.4f}")
    print(f"  K_B (Acetone):        {kb_val:.4f} ± {kb_val*error_margin:.4f}")
    print(f"  K_C (Product):        {kc_val:.4f} ± {kc_val*error_margin:.4f}")
    print(f"  K_D (Water):          {kd_val:.4f} ± {kd_val*error_margin:.4f}")
    print(f"  Temperature SSE:        {temp_ss_res:.5f}")
    print(f"  Temperature R-squared:  {temp_r_squared:.5f}")
# --- END MODIFICATION ---
...
# --- MODIFICATION: Added the SSE print statement you requested ---
print(f"\nOverall SSE (sum of squared error) of the Predictive Model (Parity): {ss_res_total:.5f}")
