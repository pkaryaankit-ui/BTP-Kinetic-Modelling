# ================================================================
#   ALDOL CONDENSATION: ELEY-RIDEAL (A adsorbed) MODEL FITTING
...
file_path = '/content/BTP data  - Sheet1.csv'  # <-- Make sure this file path is correct
...
################################################################################
# --- PART 1: FITTING ELEY-RIDEAL MODEL (A adsorbed) ---
################################################################################
#
#   *** MODIFICATION START ***
...
#
print("\n\n=====================================================================")
print("--- PART 1: Fitting Eley-Rideal (A ads) Parameters Per Temperature ---")
print("=====================================================================")

def fitting_rate_law_er_A(t, XA, T, M, CA0, k_s, K_A, K_B):
    """
    Defines the ODE for the Eley-Rideal model with A (Benzaldehyde) adsorbed.
    Rate = (ks * KA * KB * CA * CB) / (1 + KA * CA)
...
def sum_of_squared_errors_er_A(params, target_temp):
    """
    ...
    for run_data in experimental_runs.values():
        ...
        sol = solve_ivp(
            fun=lambda t, y: fitting_rate_law_er_A(
...
print("\n--- Per-Temperature Eley-Rideal (A ads) Fit Results ---")
unique_temps = sorted(list(set(raw_data['Temperature (K)'])))
for temp in unique_temps:
    print(f"Fitting for {temp} K...")
    result = minimize(
        fun=lambda p: sum_of_squared_errors_er_A(p, target_temp=temp),
        x0=initial_guess,
        method='L-BFGS-B',
        bounds=bounds
    )
    ...
    if result.success:
        ...
        fitted_params_map[temp] = {'ks': ks_fit, 'KA': KA_fit, 'KB': KB_fit}
        fitted_ks_map[temp] = ks_fit # For Arrhenius plot
        print(f"  k_s: {ks_fit:.5f} | K_A: {KA_fit:.4f} | K_B: {KB_fit:.4f}")
    else:
        print(f"  ERROR: Fit failed for {temp} K.")
        fitted_params_map[temp] = {'ks': np.nan, 'KA': np.nan, 'KB': np.nan}
...
fig1.suptitle('PART 1: Eley-Rideal (A ads) Model Fit (Per-Temp)', fontsize=18)
...
################################################################################
# --- PART 2: VALIDATING THE ELEY-RIDEAL MODEL ---
################################################################################
#
#   *** MODIFICATION START ***
...
def predictive_rate_law_er_A(t, XA, T, M, CA0, A_s, Ea, K_A, K_B):
    """
...
fig2.suptitle('PART 2: Eley-Rideal Predictive Model Validation for Aldol Condensation', fontsize=18)
...
for i, temp in enumerate(plots_data.keys()):
    ...
    for run_data in plots_data[temp]:
        ...
        t_plot = np.linspace(0, max(time_exp), 100) if max(time_exp) > 0 else [0]
        sol_plot = solve_ivp(
            fun=lambda t, y: predictive_rate_law_er_A(
                t, y, temp, run_data['M'], run_data['CA0'],
...
#
print("\n\n=======================================================")
print("--- PART 3: Final Eley-Rideal (Benzaldehyde ads) Results Summary ---")
print("=======================================================")
...
for temp, params in fitted_params_map.items():
    ...
    print(f"  K_B (Acetone const):    {kb_val:.4f} ± {kb_val*error_margin:.4f}")
...
# Parity Plot with ±10% Error Lines
