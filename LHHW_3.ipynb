# ================================================================
#   ALDOL CONDENSATION: LHHW DUAL-SITE MODEL FITTING
...
################################################################################
# --- PART 1: FITTING LHHW DUAL-SITE MODEL ---
################################################################################
#
#   *** MODIFICATION START ***
...
#
print("\n\n=====================================================================")
print("--- PART 1: Fitting LHHW Dual-Site Parameters Per Temperature ---")
print("=====================================================================")

def fitting_rate_law_lhhw_ds(t, XA, T, M, CA0, k_s, K_A, K_B):
    """
    Defines the ODE for the LHHW dual-site model with both reactants adsorbed.
    Rate = (ks * KA * KB * CA * CB) / (1 + KA * CA + KB * CB)^2
    ...
    denominator = (1 + K_A * C_A + K_B * C_B)**2
...
def sum_of_squared_errors_lhhw_ds(params, target_temp):
    """
    ...
    for run_data in experimental_runs.values():
        ...
        sol = solve_ivp(
            fun=lambda t, y: fitting_rate_law_lhhw_ds(
...
print("\n--- Per-Temperature LHHW Dual-Site Fit Results ---")
unique_temps = sorted(list(set(raw_data['Temperature (K)'])))
for temp in unique_temps:
    print(f"Fitting for {temp} K...")
    result = minimize(
        fun=lambda p: sum_of_squared_errors_lhhw_ds(p, target_temp=temp),
        x0=initial_guess,
        method='L-BFGS-B',
        bounds=bounds
    )
...
fig1.suptitle('PART 1: LHHW Dual-Site Model Fit (Per-Temp)', fontsize=18)
...
################################################################################
# --- PART 2: VALIDATING THE LHHW DUAL-SITE MODEL ---
################################################################################
#
#   *** MODIFICATION START ***
...
def predictive_rate_law_lhhw_ds(t, XA, T, M, CA0, A_s, Ea, K_A, K_B):
    """
...
fig2.suptitle('PART 2: LHHW Dual-Site Predictive Model Validation', fontsize=18)
...
for i, temp in enumerate(plots_data.keys()):
    ...
    for run_data in plots_data[temp]:
        ...
        sol_plot = solve_ivp(
            fun=lambda t, y: predictive_rate_law_lhhw_ds(
                t, y, temp, run_data['M'], run_data['CA0'],
...
#
print("\n\n=======================================================")
print("--- PART 3: Final LHHW Dual-Site Results Summary ---")
print("=======================================================")
