# ================================================================
#   ALDOL CONDENSATION: LHHW SINGLE-SITE MODEL FITTING
...
################################################################################
# --- PART 1: FITTING LHHW SINGLE-SITE MODEL ---
################################################################################
#
#   *** MODIFICATION START ***
#   We are now doing a single, global fit for all 8 parameters at once.
#   Parameters: [A_s, Ea, KA_433, KB_433, KA_443, KB_443, KA_453, KB_453]
#
print("\n\n=====================================================================")
print("--- PART 1: Global Simultaneous Fit for LHHW Single-Site ---")
print("=====================================================================")

def fitting_rate_law_lhhw_ss(t, XA, T, M, CA0, A_s, Ea, K_A, K_B):
    """
    Defines the ODE for the LHHW single-site model.
...
def sum_of_squared_errors_lhhw_ss(params):
    """
    Objective function for the global fit.
    params = [A_s, Ea, KA_433, KB_433, KA_443, KB_443, KA_453, KB_453]
    """
    total_error = 0
    A_s, Ea, KA_433, KB_433, KA_443, KB_443, KA_453, KB_453 = params

    for run_data in experimental_runs.values():
        temp = run_data['T']

        # Select the correct K_A and K_B for the current run's temperature
        if temp == 433:
            KA_run, KB_run = KA_433, KB_433
        elif temp == 443:
            KA_run, KB_run = KA_443, KB_443
        else: # 453 K
            KA_run, KB_run = KA_453, KB_453

        time_exp = np.insert(np.array(run_data['time']), 0, 0)
        conv_exp = np.insert(np.array(run_data['conversion']) / 100.0, 0, 0)

        sol = solve_ivp(
            fun=lambda t, y: fitting_rate_law_lhhw_ss(
                t, y, run_data['T'], run_data['M'], run_data['CA0'],
                A_s, Ea, KA_run, KB_run # Pass global A_s, Ea and specific K's
            ),
...
# Parameters: [A_s, Ea, KA_433, KB_433, KA_443, KB_443, KA_453, KB_453]
initial_guess = [
    1e8,     # A_s (pre-exponential factor)
    50000,   # Ea (Activation Energy in J/mol) - e.g., 50 kJ/mol
    0.5,     # KA_433
    0.5,     # KB_433
    0.5,     # KA_443
    0.5,     # KB_443
...
result = minimize(
    sum_of_squared_errors_lhhw_ss,
    initial_guess,
    method='L-BFGS-B',
    bounds=bounds
)

# Unpack the 8 fitted parameters
A_s_fit, Ea_fit, KA_433_fit, KB_433_fit, KA_443_fit, KB_443_fit, KA_453_fit, KB_453_fit = result.x

# Store results in dictionaries for plotting and tables
fitted_params_map = {
    433: {'KA': KA_433_fit, 'KB': KB_433_fit},
    443: {'KA': KA_443_fit, 'KB': KB_443_fit},
    453: {'KA': KA_453_fit, 'KB': KB_453_fit}
}
temps_K = np.array([433, 443, 453])
fitted_ks_map = {T: A_s_fit * np.exp(-Ea_fit / (R * T)) for T in temps_K}

print("\n--- Global Simultaneous Fit Results ---")
print(f"Fitted A_s: {A_s_fit:.2e}")
print(f"Fitted Ea: {Ea_fit/1000:.2f} kJ/mol")
print("\n--- Per-Temperature Fitted Constants ---")
print(f"  433 K: K_A = {KA_433_fit:.4f}, K_B = {KB_433_fit:.4f}")
print(f"  443 K: K_A = {KA_443_fit:.4f}, K_B = {KB_443_fit:.4f}")
print(f"  453 K: K_A = {KA_453_fit:.4f}, K_B = {KB_453_fit:.4f}")
...
# We don't need to re-fit the line, we can just plot the values
...
fig1.suptitle('PART 1: LHHW Single-Site Model (Global Fit)', fontsize=18)
...
for i, temp in enumerate(plots_data.keys()):
    ...
    # Get the fitted K_A and K_B for this temperature
    KA_plot = fitted_params_map[temp]['KA']
    KB_plot = fitted_params_map[temp]['KB']
    ...
    for run_data in plots_data[temp]:
        ...
        sol_plot = solve_ivp(
            fun=lambda t, y: fitting_rate_law_lhhw_ss(
                ...
                A_s_fit, Ea_fit, KA_plot, KB_plot # Use the fitted params
            ),
...
################################################################################
# --- PART 2: VALIDATING THE LHHW SINGLE-SITE MODEL ---
################################################################################
#
#   *** MODIFICATION START ***
#   Part 2 and Part 3 are now very similar, as the "predictive" model
#   is the same as the "fitted" model from Part 1.
#
print("\n\n============================================================")
print("--- PART 2: Validating Model with Fitted Parameters ---")
print("============================================================")

# This predictive function is now identical to the fitting function
def predictive_rate_law_lhhw_ss(t, XA, T, M, CA0, A_s, Ea, K_A, K_B):
...
fig2.suptitle('PART 2: LHHW Global Fit Model Validation', fontsize=18)
# ** Note: No longer need to reset all_exp_points/all_model_points here **
for i, temp in enumerate(plots_data.keys()):
    ...
    for run_data in plots_data[temp]:
        ...
        sol_plot = solve_ivp(
            fun=lambda t, y: predictive_rate_law_lhhw_ss(
                t, y, temp, run_data['M'], run_data['CA0'],
                A_s_fit, Ea_fit, KA_plot, KB_plot
            ),
            t_span=[0, max(time_exp) if max(time_exp) > 0 else 1],
            y0=[0],
            method='LSODA',
        ...
        # Plotting as a solid line because it's the "fit"
...
#
#   *** MODIFICATION START ***
#   The table is updated to pull the per-temperature K_A and K_B values
#   and the globally fitted k_s values.
#
print("\n\n=======================================================")
print("--- PART 3: Final LHHW Single-Site Results Summary ---")
print("=======================================================")

# --- MODIFICATION: Initialize lists for overall and per-temp stats ---
...
for name, run_data in experimental_runs.items():
    ...
    sol_exp_points = solve_ivp(
        fun=lambda t, y: predictive_rate_law_lhhw_ss(
            ...
            A_s_fit, Ea_fit, ka_fit_val, kb_fit_val
        ),
    ...
    # k_s_pred is the same as k_s_fit in this method
    k_pred_str = f"{k_fit_val:.5f} ± {k_fit_val*error_margin:.5f}"
    ka_str = f"{ka_fit_val:.4f} ± {ka_fit_val*error_margin:.4f}"
    kb_str = f"{kb_fit_val:.4f} ± {kb_val*error_margin:.4f}"
...
# --- BUG FIX: Corrected variable name from As_fit to A_s_fit ---
...
for temp, params in fitted_params_map.items():
    # Calculate stats for this temperature using the FITTED model
    ...
    print(f"Temp: {temp} K")
    print(f"  K_A (Benzaldehyde ads): {ka_val:.4f} ± {ka_val*error_margin:.4f}")
    print(f"  K_B (Acetone ads):        {kb_val:.4f} ± {kb_val*error_margin:.4f}")
...
print(f"\nOverall SSE (sum of squared error) of the Model (Parity): {ss_res_total:.5f}")
